...
const userProfile = () => {
    ...
    if (user_id == param) { 
        document.getElementById('add_to_friend').classList.add('d-none');
        document.getElementById('message').classList.add('d-none');
    } else {
        document.getElementById('add_to_friend').classList.add('d-lg-block');
        document.getElementById('message').classList.add('d-lg-block');
    }
    console.log(user_id);
    fetch(`http://127.0.0.1:8000/user/list/${param}`)
        .then(res => res.json())
        .then(data => {
            document.getElementById("author_name").innerHTML = data.user.first_name + " " + data.user.last_name;
            document.getElementById("author_image").src = data.profile_image;
            document.getElementById("bio").innerHTML = data.bio;
            document.getElementById("address").innerHTML = data.address;
            document.getElementById("cover_photo").style.backgroundImage = `url(${data.cover_photo})`;
